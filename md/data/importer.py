...
def run(url, destination=None, download=True):
    ...
    # Convert to CSV
    xls_path = os.path.join(destination, '2013.xls')
    csv_path = os.path.join(destination, '2013.csv')
    if not os.path.exists(csv_path):
        logger.info("Converting {} > {}".format(xls_path, csv_path))
        call(["in2csv {} > {}".format(data_path, csv_path)], shell=True)
    data_count = line_count(csv_path)
    logger.debug('2013 rows: {}'.format(data_count))
