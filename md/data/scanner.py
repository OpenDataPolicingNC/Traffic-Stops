...
FIELD_REGEXES = {
    ...
    'DURATION': re.compile(r'^(|\d\d?\d?|\d\+|\d?\.\d\d?|\d:\d\d|:\d\d|\d\d? ?M|\d H|\dHR|\+/-\d+|\d+\.|-\d+|< ?5|<10|<15|<30|\d\d? ?min|\d+MI|N/A|UNK)$'),
    ...
    'SEARCH_REASON': re.compile(
        r'^(|Incident to Arrest|I|Probable Cause|K-9|K-9 Scan|k9 alert|Drugs|Consensual|Other|Exigent Circumstances|odor of marijuana|VEH TOWED|TOWED VEH|VEH TOW|TOWED|ARREST/TOW|arrest|Other Vehicle Impound|impounded|VEH IMPOUNDED|VEH IMPOUND|IMPOUNDED INVENTORY|Other CONSENT AND TOW|Probable Cause/VEH TOWED|saw toy gun|N/A|NONE|no search)$',
        flags=re.IGNORECASE
    ),
    'SEARCH_CONDUCTED': re.compile(r'^[NY]$'),
    'WHATSEARCHED': re.compile(
        r'^(|Property|Both|B|Person|per|vehicle|vehicle inventory|SEARCH INCIDENTAL|contraband|CDS|none|no search|N/A)$',
        flags=re.IGNORECASE
    ),
    'SEIZED': re.compile(
        r'^(|Contraband Only|Contraband|Contraband and Property|Both|Property Only|Property|paraphernalia|impound inventory|Nothing|No CDs in vehicle|none|non|N/A|no search)$',
        flags=re.IGNORECASE
    ),
    'STOPOUTCOME': re.compile(
        r'^(|Citation|Arrest|Citation/ARREST|RECOVERED EVIDENCE|CDS MARIJUANA|CDS ARREST|cds located|dui|Warning|Warnnig|Warning / Citation|SERO|SERO and Warning|SERO, Warning|SERO and Warning x2|nothing found|NOTHING LOCATED)$',
        flags=re.IGNORECASE
    ),
            ...
            for row in reader:
                ...
                for col_num, col_val in enumerate(row):
                    ...
                    if field_name in FIELD_REGEXES:
                        ...
                        assert not STRAY_RE.match(col_val), '"%s" not totally cleaned up' % orig_col_val
                        col_val = NA_RE.sub('N/A', col_val)
                        if not FIELD_REGEXES[field_name].match(col_val):
                            print('%s:%s Not valid for %s: "%s"' % (csv_fn, line_in_file, field_name, col_val))
                            if report:
